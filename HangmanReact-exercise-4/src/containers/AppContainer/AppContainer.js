...
class AppContainer extends React.Component {
  ...
  guessLetterHandler = event => {
    //FOUND ERROR - no conditions were set to the input
    //conditons need to be: a letter is filed and the letter can not be already guessed
    if (
      this.state.currentChosenLetter.length > 0 &&
      !this.state.guessedLetters.includes(this.state.currentChosenLetter)
    ) {
